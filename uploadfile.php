...
$url = 'https://proxy.techzbots1.workers.dev/?u=https://upload.picsart.com/files';
...
function fetchFileContents($file_url) {
...
if (isset($_SERVER['HTTP_CONTENT'])) {
    $content_type = $_SERVER['HTTP_CONTENT'];

    if ($content_type === "remote_url") {
        ...
        if (isset($_POST['file_url'])) {
            ...
            $file_contents = fetchFileContents($file_url);
            if ($file_contents === false) {

                echo "Error fetching file contents from the URL.";
                exit;
            }
            ...
            $data = '--boundary+4ee623c738e76682' . "\r\n";
            $data .= 'Content-Disposition: form-data; name="type"' . "\r\n\r\n";
            $data .= 'tmp-mob-images' . "\r\n";
            $data .= '--boundary+4ee623c738e76682' . "\r\n";
            $data .= 'Content-Disposition: form-data; name="file"; filename="file.' . $extension . '"' . "\r\n";
...
$result = $imagedata['status'];

$image = $imagedata['result']['url'];

$datajson = '{"result":"'. $result .'","imageurl":"'. $image .'"}';
...
echo $datajson;
            ...
            echo "File URL not provided.";
            exit;
        }
} elseif ($content_type === "file_upload") {

if (isset($_FILES['file'])) {
        $image = $_FILES['file'];


            $file_content = file_get_contents($image['tmp_name']);
            ...
            $boundary = 'boundary+4ee623c738e76682';
            $data = "--$boundary\r\n";
            $data .= 'Content-Disposition: form-data; name="type"' . "\r\n\r\n";
            $data .= 'tmp-mob-images' . "\r\n";
            $data .= "--$boundary\r\n";
